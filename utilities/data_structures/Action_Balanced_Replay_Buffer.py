...
class Action_Balanced_Replay_Buffer(Replay_Buffer):
    ...
    def __init__(self, buffer_size, batch_size, seed, num_actions, data_saved_as_tensors=False, atari=False):
        self.atari = atari
    ...
    def sample_each_action_equally(self, batch_per_action):
        ...
        for action in range(self.num_actions):
            ...
            if action_memory_size == 0:
                print("No examples of action {} in buffer".format(action))
                continue
    ...
    def sample_experiences_with_certain_actions(self, allowed_actions, num_all_actions, required_batch_size):
        ...
        if isinstance(states, list):
            states = list(itemgetter(*matching_indexes)(states))
            next_states = list(itemgetter(*matching_indexes)(next_states))
        else:
        ...
        assert abs(num_points - required_batch_size) <= num_new_actions, "{} vs. {}".format(num_points, required_batch_size)
