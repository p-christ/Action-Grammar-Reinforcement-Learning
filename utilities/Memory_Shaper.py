...
class Memory_Shaper(object):
    ...
    def __init__(self, buffer_size, batch_size, seed, action_balanced_replay_buffer=True, data_saved_as_tensors=False, atari=False, device="cpu"):
        self.reset()
    ...
    def put_adapted_experiences_in_a_replay_buffer(self, action_id_to_actions):
        ...
        if self.action_balanced_replay_buffer:
            ...
            replay_buffer = Action_Balanced_Replay_Buffer(self.buffer_size, self.batch_size, self.seed, num_actions=self.num_actions,
                                                          data_saved_as_tensors=self.data_saved_as_tensors, atari=self.atari)
        else:
            print("Using ordinary replay buffer")
            replay_buffer = Replay_Buffer(self.buffer_size, self.batch_size, self.seed, data_saved_as_tensors=self.data_saved_as_tensors,
                                          atari=self.atari)
        for episode_ix in range(episodes):
            self.hindsight_action_replay(episode_ix, actions_to_action_id, replay_buffer, num_primitive_actions=self.num_actions)
        return replay_buffer


    def hindsight_action_replay(self, episode_ix, action_rules, replay_buffer, num_primitive_actions, experiences=None):
        """"Takes an episode of experiences and adds all the experiences to a replay buffer after interpreting matching
        sequences of primitive actions as macro actions"""
        if experiences is None:
            ...
            states, next_states, rewards, actions, dones = experiences

        max_action_length = np.max([len(action_sequence) for action_sequence in action_rules.keys()])
        ...
        for step in range(steps):
            ...
            for action_length in range(2, max_action_length + 1):
                ...
                assert all([action in range(num_primitive_actions) for action in action_sequence]), "All actions should be primitive here"
