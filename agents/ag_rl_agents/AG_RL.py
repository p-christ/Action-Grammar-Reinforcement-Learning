import time
from agents.ag_rl_agents.AG_DDQN_Wrapper import AG_DDQN_Wrapper
from utilities.grammar_algorithms.Grammar_Generator import Grammar_Generator
from agents.ag_rl_agents.AG_SAC_Wrapper import AG_SAC_Wrapper

class AG_RL(object):
    agent_name = "HRL"
    def __init__(self, config):
        assert config.hyperparameters["Base_Agent"] in ["DDQN", "SAC"], "Base agent must be DDQN or SAC"
        if config.hyperparameters["Base_Agent"] == "DDQN": self.agent = AG_DDQN_Wrapper(config)
        else: self.agent = AG_SAC_Wrapper(config)
        self.config = config
