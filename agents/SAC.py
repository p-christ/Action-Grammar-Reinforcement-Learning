...
class SAC(Base_Agent):
    ...
    def __init__(self, config):
        ...
        if self.automatic_entropy_tuning:
            self.target_entropy = -torch.prod(torch.Tensor(self.environment.action_space.shape).to(self.device)).item() # heuristic value from the SAC paper
        ...
        if "reward_scale" in self.hyperparameters.keys():
            self.reward_scale = self.hyperparameters["reward_scale"]
        else:
            self.reward_scale = 1.0

    def prepare_state(self, states):
        """Prepares state for input into a network"""
        if "Atari" in self.environment_title:
    ...
    def play_episode(self, eval_ep=None, save_experience_override=False):
        """Runs an episode on the game, saving the experience and running a learning step if appropriate"""
        self.episode_rewards = []
        if eval_ep is None:
            eval_ep = self.episode_number % TRAINING_EPISODES_PER_EVAL_EPISODE == 0 and self.do_evaluation_iterations
        self.first_step_of_ep = True
        while not self.done:
            ...
            if not eval_ep:
                ...
                if isinstance(self.hyperparameters["Critic"]["tau"], float):
                    self.soft_update_of_target_network(self.critic_local, self.critic_target,
                                                       self.hyperparameters["Critic"]["tau"])  # Update the target network
                    self.soft_update_of_target_network(self.critic_local_2, self.critic_target_2,
                                                       self.hyperparameters["Critic"]["tau"])  # Update the target network
                elif self.global_step_number % self.hyperparameters["Critic"]["update_fixed_network_freq"] == 0:
                    print("------------------")
                    print("Updating Fixed Network")
                    print("------------------")
                    self.copy_model_over(self.critic_local, self.critic_target)
                    self.copy_model_over(self.critic_local_2, self.critic_target_2)
    ...
    def produce_action_and_action_info(self, state):
        ...
        assert not torch.isnan(actor_output).any(), actor_output

        mean, log_std = actor_output[:, :self.action_size], actor_output[:, self.action_size:]

        # Must do this in order to stabilise training
        log_std = torch.clamp(log_std, LOG_STD_MIN, LOG_STD_MAX)

        std = torch.exp(log_std)

        assert not torch.isnan(mean).any(), mean
        assert not torch.isnan(std).any(), std

        normal = Normal(mean, std)

        pre_tanh_value = mean + std * Normal(torch.zeros(mean.size()), torch.ones(std.size())).sample().detach()
        pre_tanh_value.requires_grad_()
        action = torch.tanh(pre_tanh_value)
        log_prob = normal.log_prob(pre_tanh_value) - torch.log(1 - action * action + EPSILON)
    ...
    def time_for_critic_and_actor_to_learn(self):
        ...
        actor and critic"""
        return self.global_step_number > self.hyperparameters["initial_random_steps"] and \
    ...
    def learn(self):
        ...
        state_batch = self.prepare_state(state_batch)
        next_state_batch = self.prepare_state(next_state_batch)
    ...
    def calculate_actor_loss(self, state_batch):
        ...
        assert not torch.isnan(log_pi).any(), log_pi
    ...
    def update_all_parameters(self, critic_loss_1, critic_loss_2, actor_loss, alpha_loss):
        ...
        if self.first_step_of_ep:
            print("Critic loss {} -- Critic loss {} -- Actor loss {} -- Alpha loss {} -- Alpha {}".format(critic_loss_1, critic_loss_2, actor_loss, alpha_loss, self.alpha))
